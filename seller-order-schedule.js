(()=>{
  ...
  function renderSlots(block,date){
    ...
    list.innerHTML=slots.map((slot,index)=>'<article class="appointment-slot-row" data-index="'+index+'"><div class="slot-time-range"><b>'+toLabel(slot.start)+' - '+toLabel(slot.end)+'</b></div><label class="slot-capacity-field">Slots<input type="number" min="1" step="1" value="'+(slot.capacity||10)+'"></label><button type="button" class="remove-slot">Remove</button></article>').join('')||'<p class="schedule-empty">No slots yet. Add a time slot for this date.</p>';
  ...
  function enhanceSettings(){
    ...
    block.innerHTML='<div class="appointment-head"><div><b>Online order appointment schedule</b><p>Select one calendar date, then set available pickup/order hours and slots for that day.</p></div><label>Calendar date<input id="appointmentDate" type="date" value="'+selected+'"></label></div><div class="appointment-builder"><label>Start time<input id="slotStart" type="time" value="08:00"></label><label>End time<input id="slotEnd" type="time" value="09:00"></label><label class="slot-capacity-field">Slots<input id="slotCapacity" type="number" min="1" step="1" value="10"></label><button type="button" class="add-appointment-slot">Add time slot</button></div><div class="appointment-slot-list"></div><span class="schedule-save-status"></span>';
    panel.append(block);
