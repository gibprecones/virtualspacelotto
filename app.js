const permitSelect=document.getElementById('hasPermit'),supportingDocLabel=document.getElementById('supportingDocLabel');
const getApplications=()=>JSON.parse(localStorage.getItem('vslApplications')||'[]');
const saveApplications=applications=>localStorage.setItem('vslApplications',JSON.stringify(applications));
function seedDemoApplicant(){const applications=getApplications();if(applications.some(item=>item.email==='demo.seller@virtualspacelotto.com'))return;applications.push({id:'VSL-DEMO-0001',name:'Demo Seller',email:'demo.seller@virtualspacelotto.com',mobile:'09171234567',address:'Manila, Philippines',hasPermit:'yes',status:'APPROVED',followUp:false,password:'Seller123!'});saveApplications(applications)}
seedDemoApplicant();
function showModal(id){document.getElementById(id).style.display='grid'}function hideModal(id){document.getElementById(id).style.display='none'}function showLogin(){showModal('login')}function hideLogin(){hideModal('login')}function hideSupport(){hideModal('support')}function hidePasswordSetup(){hideModal('passwordSetup')}
function requestFollowUp(){const id=localStorage.getItem('vslApplication'),applications=getApplications(),application=applications.find(item=>item.id===id);if(application){application.followUp=true;saveApplications(applications)}showModal('support')}
permitSelect.addEventListener('change',()=>{supportingDocLabel.childNodes[0].nodeValue=permitSelect.value==='yes'?'Business Permit ':'NBI / supporting document '});
