...
async function startCam(){try{stream=await navigator.mediaDevices.getUserMedia({video:true,audio:true});cam.srcObject=stream;empty.style.display='none';badge.textContent='🔴 LIVE';badge.classList.add('live')}catch(error){alert('Camera permission is required. Use HTTPS or localhost.')}}
